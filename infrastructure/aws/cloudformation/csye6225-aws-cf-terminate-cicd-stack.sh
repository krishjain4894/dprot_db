...
lambda_bucket_id=$(aws cloudformation describe-stack-resources --stack-name $var1 --logical-resource-id S3BucketForLambda --query "StackResources[0].PhysicalResourceId" --output text)
aws s3 rm 's3://'$lambda_bucket_id --recursive
